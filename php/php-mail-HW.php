...
<body>
 	...
 	<form method="post" action="<?php echo $_SERVER["PHP_SELF"];?>">
            ...
            <input name="lname" placeholder="Type Here">
 ...
 <?php   
    ...
    if (isset($_POST["submit"])){ 
    	$to = "sd5936@rit.edu"; // !!! REPLACE WITH YOUR EMAIL !!!
    	...
    	// else, use the value from the form field
			...
			$name = empty(trim($_POST["name"])) ? "No first name" : sanitize_string($_POST["name"]);
            
            $lname = empty(trim($_POST["lname"])) ? "No last name" : sanitize_string($_POST["lname"]);
